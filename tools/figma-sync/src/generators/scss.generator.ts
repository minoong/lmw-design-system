 ...
 */
export function generateTokensSCSS(collection: ParsedTokenCollection): string {
  const lines: string[] = [];
 ...
 */
export function generateTokensIndexSCSS(collections: ParsedTokenCollection[]): string {
  const lines: string[] = [];
  lines.push(generateHeader());
  lines.push('');
 ...
 */
export function generateTypographySCSS(typography: ParsedTypography[]): string {
  const lines: string[] = [];
  lines.push(generateHeader());
  lines.push('');
  ...
  lines.push('  --ds-font-family-sans: "Pretendard", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;');
  lines.push('  --ds-font-family-mono: "JetBrains Mono", "Fira Code", Consolas, monospace;');
  lines.push('');
 ...
 */
export function generateThemeSCSS(themeName: string, tokens: ParsedTokenCollection[]): string {
  const lines: string[] = [];
  lines.push(generateHeader());
  lines.push('');

  // Find semantic tokens for theme
  const semanticTokens = tokens.find((c) => c.name.toLowerCase().includes('semantic'));
